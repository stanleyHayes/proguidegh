...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  return <div className="admin-frame">
    ...
    {tourStep !== null && currentTour && <div className="admin-tour" role="dialog" aria-modal="true" aria-labelledby="tour-title"><div className="admin-tour__card"><span>STEP {tourStep + 1} OF {tour.length}</span><button aria-label="Close walkthrough" onClick={closeTour}>×</button><div className="admin-tour__guide">PG</div><h2 id="tour-title">{currentTour[0]}</h2><p>{currentTour[1]}</p><footer>{tourStep > 0 ? <button onClick={() => setTourStep(tourStep - 1)}>Back</button> : <button onClick={closeTour}>Skip</button>}<button className="is-primary" onClick={() => tourStep === tour.length - 1 ? closeTour() : setTourStep(tourStep + 1)}>{tourStep === tour.length - 1 ? "Finish" : "Next"}</button></footer></div></div>}
  </div>;
}
