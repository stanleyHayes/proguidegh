/**
 * Certification pipeline constants shared by the guide screens (spec §5).
...
const STAGE_LABELS: Record<string, string> = {
  ...
  ACTIVE: "Active — open for bookings",
  REJECTED: "Rejected",
  SUSPENDED: "Suspended",
  EXPIRED: "Expired",
...
/** Index of the current status in the happy-path pipeline, or -1. */
export function stageIndex(status?: string | null): number {
  return PIPELINE_STAGES.indexOf(normalizeStatus(status) as PipelineStage);
}

export function isExceptionStatus(status?: string | null): boolean {
  return (EXCEPTION_STATUSES as readonly string[]).includes(
