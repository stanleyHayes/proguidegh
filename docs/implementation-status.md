...
- 2026-08-14 — Admin shell/account architecture pass: reviewed RentOS `DashboardLayout`, header account/notification menus, onboarding tour and tabbed settings patterns. Login now renders outside console chrome; the desktop rail is viewport-fixed with only its navigation region independently scrollable; mobile retains the connected off-canvas rail. Added account and notification popovers, a persisted first-run/replayable three-step walkthrough, and `/admin/account` tabs for profile, security/password reset, device preferences and notification choices. Sensitive profile mutation is intentionally read-only because no audited current-admin update endpoint exists; password reset uses the real uniform `/auth/password/forgot` flow. Admin typecheck, lint, 21-route production build and `git diff --check` exited 0. Live Chrome verified standalone login, `position: fixed` rail with hidden outer overflow and auto-scrolling nav, account route/tab rendering, onboarding dialog, and the 390px mobile drawer.
- 2026-08-14 — Access/onboarding correctness review: traced admin access and guide registration against the live handlers and recorded findings in `docs/reviews/2026-08-14-access-onboarding-review.md`. Added migration 0012 plus audited 72-hour single-use admin invitations, protected invitation directory/create endpoints, public acceptance, replay protection, multi-role access review and self-lockout guards. Repaired guide application, region, language, evidence-upload and dashboard response contracts; completed admin/guide MFA challenge UI and real admin authenticator enrollment. Verification evidence is appended after the final gate below.
- 2026-08-14 | access/onboarding review | temp PostgreSQL `migrate up` (0012), `down` (0012), `up` (0012); `pg_indexes` query; API integration suite with live PostgreSQL/Redis; `go test ./...`; `go vet ./...`; OpenAPI dump diff + contract generation; workspace `pnpm lint`, `pnpm typecheck`, `pnpm build`; `git diff --check`; HTTP smoke on API + ports 3000–3003 | PASS — five invitation indexes present; single-use/audit/role integration passed; all Go packages green; 12 workspace lint/typecheck projects green; four Next apps built including `/invite/[token]`; API readiness and nine web routes returned 200 | `docs/reviews/2026-08-14-access-onboarding-review.md`
